	...
	public class CarouselView<T> : ContentView
	{
        ...
        public CarouselView()
        {
            ...
            this.interests.Add(new GestureInterest
            {
                ...
                GestureCommand = new RelayGesture((g, x) => SwitchView(false), (g, x) => this.currentPageNum > 0)
            });
            this.interests.Add(new GestureInterest
            {
                Direction = Directionality.Left,
                GestureType = GestureType.Swipe,
                GestureCommand = new RelayGesture((g, x) => SwitchView(true), (g, x) => this.currentPageNum < ViewModels.Count)
            });
        ...
        {
            var newval = this.currentPageNum + (increment ? 1 : -1);
            
            if (newval < 0 || newval > ViewModels.Count() - 1) return;
            var oldview = this.currentPageNum >=0 ?  ViewModels[this.currentPageNum] as ICarouselView : null;
            ...
            this.contentView.ViewModel = ViewModels[this.currentPageNum];
            if(oldview != null) oldview.Hiden();
            if(newview != null) newview.Shown();
            this.myGrid.Children.Add(this.marker, this.currentPageNum, 0);
        }
        ...
        private void ViewModelCollectionContentsChanged(object sender, NotifyCollectionChangedEventArgs e)
        {
            ...
            switch (e.Action)
            {
                ...
                case NotifyCollectionChangedAction.Remove:
                    if (e.OldStartingIndex == this.currentPageNum) //Well damn
                        SwitchView(this.currentPageNum == 0);
                    break;
                case NotifyCollectionChangedAction.Add:
                    if (e.NewStartingIndex == this.currentPageNum)
                        SwitchView(this.currentPageNum < ViewModels.Count);
                    break;
            }
        }
